...
#define FEDORA_ID 5
  ...
  #elif (RHE_MJ==8) && (RHE_MN==0)
    // TODO: Use RH_REL to determine the definition instead of RH_MJ/MN when official release
    // kernel 4.18.0-147 replace access_ok(x,y,z) to access_ok(y,z)
    #if (RH_REL<147)
      #define ACCESS_OK(x,y,z) access_ok(x,y,z)
    #else
  ...
  // -> Fedora
  #if (DIST_ID==FEDORA_ID)
    #if (LINUX_VERSION_CODE >= VERSION_CODE(4,18,16))
      #undef ACCESS_OK
      //For Fedora 29 (4.18.16) which has transferred access_ok to __access_ok() which always return 1
      #define ACCESS_OK(x,y,z) __ACCESS_OK(y,z)
static inline int __ACCESS_OK(void* addr, unsigned long size){return 1;}
    #endif
  #endif

  // -> Kylin
  #if (DIST_ID==KYLIN_ID)
    #if (LINUX_VERSION_CODE >= VERSION_CODE(4,19,90))
      #undef ACCESS_OK
      #define ACCESS_OK(x,y,z) access_ok(y,z)
    #endif
  #endif
...
// RHEL8.6, kernel 4.18.0-372 replace current->state to current->__state.
// However RHEL8.5, kernel 4.18.0-348 didn't do that.
#ifdef RHEL8
  #if (RHE_MJ<8)
    #define _KSTATE state
  #elif (RHE_MJ==8) && (RHE_MN<6)
    // TODO: Use RH_REL to determine the definition instead of RH_MJ/MN when official release
    #if (RH_REL<372)
      #define _KSTATE state
    #else
...
static void npreal_flush_to_ldisc(struct work_struct *work)
{
	...
	if ( tty && (IS_TTY_INIT(&info->ttyPort)))
	{
#if (LINUX_VERSION_CODE < VERSION_CODE(3,8,0))  
